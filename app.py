...
# 2. Pop-up Units (Standard Defaults)
MARQUEE_UNITS = {
...
# 3. Flooring & General Accessories
GENERAL_PRODUCTS = {
    "Flooring": {
        "Supa-trac flooring": {"rate": 11.55, "block": 25.00, "lab_perc": 0.40, "kg_sqm": 4.5, "unit": "SQM"},
        "I-Trac flooring": {"rate": 23.40, "block": 46.80, "lab_perc": 0.40, "kg_sqm": 15.0, "unit": "SQM"}
    },
    "Accessories": {
        "Orange Weight (Individual)": {"rate": 6.60, "lab_perc": 0.25, "kg": 30, "unit": "ea"}
    }
}
...
# --- APP UI ---
st.set_page_config(page_title="No Fuss Quote Pro v29.4", layout="wide")
...
# --- SECTION 1: MARQUEES & STRUCTURES ---
st.markdown("### ⚡ SMART MARQUEE QUICK-ADD")
...
if st.button("ADD PRODUCT"):
    ...
    st.session_state.df = pd.concat([st.session_state.df, pd.DataFrame([{
        ...
        "Weight_KG": kg_total, "Is_Lab_Line": False,
        "Hire_Math_Str": f"{qty_val} - {product_name} x ${rate_to_use:,.2f} = ${h_total:,.2f}"
...
if not st.session_state.df.empty:
    ...
    for idx, row in st.session_state.df.iterrows():
        # Note: Hire duration logic already applied at entry for flooring/general, but structures need live_weeks check
    ...
    st.markdown(f"**🚚 Payload:** {total_weight:,.0f} kg | **Trucks:** {truck_count} x 6,000kg")
