...
# --- UI ---
st.set_page_config(page_title="v30.1 Unified Quoter", layout="wide")
if 'df' not in st.session_state:
    st.session_state.df = pd.DataFrame(columns=["Qty", "Product", "Unit Rate", "Total", "Min_Lab", "Raw_Lab", "Lab_Math", "KG", "Is_Marquee", "Hire_Math"])

st.title("📦 No Fuss Unified Quoter")
...
# --- INPUT AREA (ONE SCREEN) ---
col_mq, col_fl = st.columns(2)

with col_mq:
    st.markdown("### ⚡ Add Marquees")
    m_in = st.text_input("Size (Span x Length) - e.g. 4x12")
    ...
    if st.button("Add Marquee"):
        ...
        if len(nums) >= 2:
            ...
            if span == 3 and (length == 3 or length == 6):
                ...
                new_rows.append({"Qty": m_q, "Product": key, "Unit Rate": data['rate'], "Total": 0.0, "Min_Lab": data['min'], "Raw_Lab": l_val, "Lab_Math": f"{key}: ${h_val:,.2f} x 55% = ${l_val:,.2f}", "KG": data['kg']*m_q, "Is_Marquee": True, "Hire_Math": f"{m_q} - {key} x ${data['rate']}"})
                legs = data['legs']
            else:
                ...
                new_rows.append({"Qty": m_q, "Product": f"Structure {span}x{length}", "Unit Rate": sqm*rate, "Total": 0.0, "Min_Lab": logic['min_lab'], "Raw_Lab": l_val, "Lab_Math": f"{span}x{length}: ${h_val:,.2f} x {int(lab_p*100)}% = ${l_val:,.2f}", "KG": (sqm*2.5)*m_q, "Is_Marquee": True, "Hire_Math": f"{m_q} - Structure {span}x{length} ({sqm}sqm x ${rate})" })
                legs = ((length/logic['bay'])+1)*2
            if m_sec == "Weights":
                w_tot = int(legs*6*m_q); w_h = w_tot*6.60; w_l = w_h*0.25
                new_rows.append({"Qty": w_tot, "Product": "30kg Weights", "Unit Rate": 6.60, "Total": 0.0, "Min_Lab": 0, "Raw_Lab": w_l, "Lab_Math": f"Weights: ${w_h:,.2f} x 25% = ${w_l:,.2f}", "KG": w_tot*30, "Is_Marquee": True, "Hire_Math": f"{w_tot} - Weights x $6.60"})
...
with col_fl:
    ...
    f_name = st.selectbox("Floor Type", list(FLOORING_LOGIC.keys()))
    f_qty = st.number_input("SQM Amount", min_value=0.0)
    if st.button("Add Flooring"):
        ...
        st.session_state.df = pd.concat([st.session_state.df, pd.DataFrame([{
            "Qty": f_qty, "Product": f_name, "Unit Rate": f_rate, "Total": 0.0, "Min_Lab": 0, "Raw_Lab": l_val, "Lab_Math": f"{f_name}: ${h_val:,.2f} x 40% = ${l_val:,.2f}", "KG": f_qty*data['kg'], "Is_Marquee": False, "Hire_Math": f"{f_qty}sqm - {f_name} x ${f_rate:,.2f}"
        }])], ignore_index=True); st.rerun()

# --- QUOTE DISPLAY ---
if not st.session_state.df.empty:
    st.divider(); st.markdown("### 🏗️ Current Quote")
    ...
    for idx, row in st.session_state.df.iterrows():
        # Apply Hire Duration only to Marquee Logic
        ...
        total_weight_line = row["KG"]
        total_kg += total_weight_line; st.session_state.df.at[idx, "Total"] = line_h
        details.append(row["Lab_Math"])
    ...
    st.markdown(f"**🚚 Payload:** {total_kg:,.0f}kg | **Trucks Required:** {trucks}")
