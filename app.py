...
with col2:
    ...
    p_sel = st.selectbox("Product", list(FLOORING_DATA.keys()))
...
if not st.session_state.df.empty:
    ...
    l_maths = [f"Damage Waiver (7%): ${h_wk1_gear:,.2f} x 0.07 = ${wav:,.2f}", f"Cartage: {trucks} Trucks x {cur_km}km x 4 x $3.50 = ${crt:,.2f}"]
