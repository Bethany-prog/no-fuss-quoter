...
def load_project_safe(fname):
    try:
        with open(f"quotes/{fname}", "r") as f:
            ...
            km_val = d.get("km", 0.0)
            st.session_state.km = float(km_val) if km_val is not None else 0.0
...
# FIX: Safe input comparison
km_in = st.session_state.km
km_shw = km_in if (km_in is not None and km_in > 0) else None
...
if not st.session_state.df.empty:
    ...
    # FIX: SAFE TRUCK & KM CALCULATION
    truck_count = math.ceil(total_kg / 6000) or 1
    safe_km = st.session_state.km if (st.session_state.km is not None) else 0.0
    
    wav = h_wk1_gear * 0.07
    crt = truck_count * safe_km * 4 * 3.50 if cartage_mode == "Charge" else 0.0
    lab = max(st.session_state.df["Raw_Lab"].sum(), 350) if labour_mode == "Separate" else 0.0
    
    l_maths = [f"Damage Waiver (7%): ${h_wk1_gear:,.2f} x 0.07 = ${wav:,.2f}", f"Cartage: {truck_count} Trucks x {safe_km}km x 4 x $3.50 = ${crt:,.2f}"]

    st.divider(); m = st.columns(6)
    m[0].metric("HIRE", f"${h_tot_c:,.2f}"); m[1].metric("LABOUR", f"${lab:,.2f}"); m[2].metric("WAIVER", f"${wav:,.2f}"); m[3].metric("CARTAGE", f"${crt:,.2f}"); m[4].metric("WEIGHT", f"{total_kg:,.0f}kg"); m[5].metric("TRUCKS", f"{truck_count}")
