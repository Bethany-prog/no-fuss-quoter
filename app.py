...
# --- v32.7 AUS DATE & CHECKLIST UPDATE ---
if not os.makedirs("quotes", exist_ok=True): pass
...
st.markdown("""
    ...
    /* Checklist Styling */
    .check-box { background-color: #F1F3F4; padding: 15px; border-radius: 8px; border-left: 5px solid #3D5AFE; margin-bottom: 10px; }
    </style>
...
# --- UI WORKFLOW ---
st.title("📦 No Fuss Unified Engine (v32.7)")
...
start_d = c1.date_input("Hire Start (DD/MM/YYYY)", value=date.today(), format="DD/MM/YYYY")
end_d = c2.date_input("Hire End (DD/MM/YYYY)", value=date.today(), format="DD/MM/YYYY")
...
# ... (Marquee and Flooring logic remains locked as per v32.6) ...
with col_mq:
...
with col_cat:
    ...
    if st.button("Add to Quote"):
        ...
        h_val = f_qty * f_rate; l_val = f_qty * data['lab_fix'] if 'lab_fix' in data else (h_val * data.get('lab_p', 0))
...
if not st.session_state.df.empty:
    ...
    for idx, row in st.session_state.df.iterrows():
        ...
        h_tot += line_h; raw_l_sum += row["Raw_Lab"]; total_kg += row["KG"]; st.session_state.df.at[idx, "Total"] = line_h
    
    trucks = math.ceil(total_kg / CONFIG["TRUCK_PAYLOAD"]) if total_kg > 0 else 1
    final_lab = raw_l_sum; waiver = h_tot * 0.07; cartage = trucks * km_in * 4 * CONFIG["CARTAGE_RATE"]; grand = h_tot + final_lab + waiver + cartage
    
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("HIRE", f"${h_tot:,.2f}"); m2.metric("LABOUR", f"${final_lab:,.2f}"); m3.metric("LOAD", f"{total_kg:,.0f}kg"); m4.metric("TRUCKS", f"{trucks}")
    
    # --- DYNAMIC CHECKLIST ---
    st.markdown("### 🛠️ Project Guardrails (Checklist)")
    with st.container():
        st.markdown("<div class='check-box'>", unsafe_allow_html=True)
        if any("Structure" in item for item in quote_items):
            st.checkbox("Structures: Have you confirmed Pegging vs Weights with the client?", value=False)
            st.checkbox("Logistics: Is there clear truck access for 6-tonne vehicles?", value=False)
        if any("I-Trac" in item for item in quote_items):
            st.checkbox("I-Trac: Have you included Ramps for all entry points?", value=False)
        if any("Plastorip" in item for item in quote_items):
            st.checkbox("Plastorip: Check for Edging and Corner requirements.", value=False)
        if any("Supa-Trac" in item for item in quote_items):
            st.checkbox("Supa-Trac: Are there enough edging pieces for the perimeter?", value=False)
        st.checkbox("Admin: Has the Damage Waiver been explained to the customer?", value=False)
