...
# --- PDF GENERATION (v25.8 - RAW MATH STRINGS) ---
def create_calculation_pdf(name, df, subtotal, labour, waiver, cartage, grand_total, km, weeks):
    ...
    for _, row in df.iterrows():
        ...
        # Build specific math for the Breakdown section below
        if row['Is_Lab_Line']:
            # Example: ($414.00) x 55% = $227.70
            # Note: The Unit Rate of the labour line is the 55% value
            struct_total = row['Total'] / 0.55 if row['Total'] > 0 else 0
            labour_math_lines.append(f"(${struct_total:,.2f}) x 55% = ${row['Total']:,.2f}")
        else:
            # Example: (6 x 3) x $23.00 x 1
            if "Marquee" in row['Product']:
                # Extract dimensions from product name (e.g. "Marquee 6 X 3")
                dims = row['Product'].replace("Marquee ", "")
                hire_math_lines.append(f"({dims}) x $23.00 x {row['Qty']}")
            else:
                hire_math_lines.append(f"{row['Qty']} x ${row['Unit Rate']:,.2f}")
    ...
    pdf.cell(0, 5, " + ".join(hire_math_lines), ln=True)
    
    # 2. Labour
    pdf.ln(2); pdf.set_text_color(0, 0, 0); pdf.set_font("Arial", "B", 11)
    pdf.cell(100, 8, "Labour:", 0); pdf.cell(0, 8, f"${labour:,.2f}", 0, 1, "R")
    pdf.set_font("Arial", "I", 9); pdf.set_text_color(100, 100, 100)
    for l_line in labour_math_lines:
        pdf.cell(0, 5, l_line, ln=True)
...
# 3. STYLING & CATALOG (Synced to v25.7)
