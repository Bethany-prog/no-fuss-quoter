...
# --- PDF ENGINE (ULTRA-SAFE) ---
def clean_text(txt):
    """Aggressively cleans text to prevent FPDF Unicode crashes."""
    if not txt: return ""
    replacements = {"®": "(R)", "™": "(TM)", "©": "(C)", "└": "->", "—": "-", "–": "-"}
    cleaned = str(txt)
...
st.markdown("""
    ...
    div.stMetric { background-color: #FFFFFF !important; padding: 20px !important; border-radius: 12px !important; border: 2px solid #3D5AFE !important; }
...
def load_project_safe(fname):
    try:
        with open(f"quotes/{fname}", "r") as f:
            ...
            st.session_state.km = float(d.get("km", 0.0)) if d.get("km") is not None else 0.0
...
# CRASH FIX: Explicit None-Check
k_in = st.session_state.km
km_to_show = k_in if (k_in is not None and k_in > 0) else None
st.session_state.km = c3.number_input("One-Way KM", value=km_to_show, placeholder="KM...")
...
if not st.session_state.df.empty:
    ...
    cur_km = st.session_state.km if st.session_state.km else 0
    wav, crt = h_wk1_gear * 0.07, trks * cur_km * 4 * 3.50 if cartage_mode == "Charge" else 0
    lab = max(st.session_state.df["Raw_Lab"].sum(), 350) if labour_mode == "Separate" else 0
    l_maths = [f"Damage Waiver (7%): ${h_wk1_gear:,.2f} x 0.07 = ${wav:,.2f}", f"Cartage: {trks} Trucks x {cur_km}km x 4 x $3.50 = ${crt:,.2f}"]
